...
- 代理健康节点的 TeamNode 注册、心跳和下线请求；公网路线异常的节点只记录最小 UUID 隔离信息，不进入监控面板，也不转发到上游；
...
节点筛选包含“全部、在线、超时、离线”四类。Worker 为异常节点维护独立的最小 UUID 隔离记录：一旦 Cloudflare Tunnel 或直连公网路线被明确判定为不可用，Durable Object 只保存 UUID、首次/最后拦截时间、10 分钟恢复期限和拦截次数，并立即删除同 UUID 的可见节点记录。异常注册、心跳和下线不会保存节点详情，不会出现在面板和 `/api/nodes`，也不会向 `teamnode.lemon.vin` 发起对应请求。

隔离后的相同 UUID 即使继续发送异常或状态不明的上报，也会在门禁阶段直接丢弃；缺少连通性字段不能绕过隔离。首次隔离会建立 10 分钟恢复观察期，超过 10 分钟仍未恢复时隔离不会自动过期，后续同 UUID 上报仍继续丢弃。只有节点明确上报最终公网路线已恢复，Worker 才允许重新尝试 TeamNode 同步；上游返回成功后才原子解除 UUID 隔离、重新写入可见记录并展示。如果 TeamNode 已将旧记录移除，节点会按现有 404 处理自动重新注册。

累计心跳少于 5 次的健康节点，在明确下线或 Worker 确认其超过心跳检测阈值后直接删除，不进入超时/离线保留流程。累计达到 5 次心跳的节点，明确下线后的前 5 分钟显示“超时”，第 5–10 分钟显示“离线”，超过 10 分钟后自动删除。没有明确下线通知的节点，Worker 会在下一次轮询发现其超过 5 分钟没有心跳时显示“离线”，并在最后一次活动超过 10 分钟后自动删除。Worker 不会主动替节点生成心跳。可用 Worker 变量 `TEAMNODE_DASHBOARD_HEARTBEAT_TIMEOUT_MS` 调整心跳阈值；可用 `TEAMNODE_DASHBOARD_ONLINE_TTL_MS` 调整自动删除时间，但必须不小于 30 秒。
...
最新版安装器在覆盖已有安装且未指定 `UUID` 时，会自动复用旧 `.env` 中的 UUID，这样面板中的节点身份不会因为重新安装而改变。如果旧节点已经通过最终公网路线验证，安装器还会在删除旧目录前保存上次的直连模式、地址族和端口作为复验提示；Tunnel 仍然优先，只有 Tunnel 不可用时才由 Worker 重新回访上次成功的直连地址族和端口。复验通过会立即复用，不再为不可达的另一地址族继续扫描全部扩展端口；复验失败才执行完整端口发现。旧配置只是提示，不会跳过公网验证。仍建议先读取并显式传入旧 UUID，不要直接打印整个 `.env`：
...
如果相同 UUID 在 TeamNode 中仍有原节点记录，重新注册返回冲突时节点会自动改用心跳复用原记录；如果原记录已经被删除，现有的心跳 `404` 流程会自动重新注册。因此成功使用过的机器重跑安装器时，不需要为了避免重复记录而更换 UUID。
...
6. 单端口公网 TCP 回访硬超时为 3.5 秒；发现任一地址族存在可用端口并完成另一地址族同端口复验后立即停止后续批次，在 `finally` 阶段关闭临时监听。不会为了寻找后续的双栈共同端口而牺牲已经验证成功的单栈路线。

Worker 接口按每个地址族最多接受每批 4 个端口，为 Cloudflare Worker 的并发外连限制保留余量；IPv4/IPv6 使用独立、按顺序执行的请求。IPv4 找到端口后，IPv6 请求只包含同一个端口，而不是重复检查整批。默认选择顺序是 `80,8080,8443,8880,2053,2083,2087,2096,443`，然后才是其余自定义或扩展结果；非标准端口被选中时使用普通 HTTP。`SERVER_PORT`（默认 3000）和 `ARGO_PORT`（默认 8001）始终排除，扩展扫描还会排除 `22,25,53,110,143,587,3306,3389`。

将 `DIRECT_PORT_SCAN_MAX` 设置为 `0` 可关闭扩展扫描，最大值为 `4096`。默认 256 个端口在单一地址族全部表现为超时的理论最坏情况下约需 `256 / 4 × 3.5 = 224` 秒；双栈机器如果两种地址族的所有端口都超时，顺序检查理论上最多约为 448 秒，加上批次请求开销会超过安装器默认的 180 秒等待时间。需要确认两种地址族都没有任何可用端口时，建议同时设置：
...
如果公网路线异常，则会改为出现“UUID 已隔离，不进入监控面板，也未向 TeamNode 推送”或“本次心跳已丢弃，未向 TeamNode 推送”；这是预期的隔离行为。
...
- 在删除旧目录前读取旧 `.env` 中的 UUID，用于保持节点身份；如果存在 `.route-ready` 最终验证标记，还会提取旧直连地址族和端口作为本次公网复验提示；
...
`ARGO_AUTH`、TeamNode 中继令牌或其他旧配置行。旧 UUID 和已通过最终验证的直连路线只会在内存中暂时提取；路线会写成 `DIRECT_REUSE_*` 复验提示，必须再次通过 Worker 公网回访后才能使用，不会直接复制或盲目信任旧配置。
