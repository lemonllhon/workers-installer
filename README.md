...
每次正式安装都会先停止本安装器创建的服务、runner、PM2，删除旧的 `${APP_DIR}`，并检查 `SERVER_PORT`、`ARGO_PORT`、Xray 的 `3001-3004` 监听端口。只有命令行明确属于本安装目录的旧进程会自动终止；若端口属于其他程序，安装会停止并提示，避免误杀。确认要强制清理其他程序时，才设置 `FORCE_KILL_PORTS=true`。
